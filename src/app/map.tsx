...
export default function MapScreen() {
  ...
  // Estandes ocupados (número normalizado) — reflete as atribuições do organizador.
  ...
  function selectStandFromMap(nextStandNumber: string) {
    const booth = exhibitors.find((item) => standNumber(item.stand) === nextStandNumber.replace(/\D/g, ''));
      ...
      <ScrollView
        ...
        <View style={styles.header}>
          <View style={styles.headerIcon}>
            <Ionicons name="map" size={22} color={Brand.gold} />
          </View>
          <View style={styles.headerCopy}>
            <Text style={styles.eyebrow}>Planta oficial</Text>
            <Text style={styles.title}>Mapa da Expoindustrial Sul</Text>
            <Text style={styles.subtitle}>
              {source === 'firestore'
                ? 'Expositores publicados sincronizados com o portal da feira.'
                : 'Visualize auditórios, estandes premium, ilhas de expositores, serviços e entrada principal.'}
            </Text>
          </View>
          <Pressable
            accessibilityLabel="Abrir mapa 3D"
            style={styles.testMapButton}
            onPress={() => router.push('/map-3d' as any)}>
            <Ionicons name="sparkles" size={18} color={Brand.gold} />
            <Text style={styles.testMapButtonText}>Visualizar 3D</Text>
          </Pressable>
        </View>
